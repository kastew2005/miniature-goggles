...
class Game{
 ...
 async start(){
  ...
  try{
    ...
    try{ this.world.processMeshQueue(this.quality.mobile?18:35); }
    ...
    this.running=true;
    this.setGameUI(true);
    requestAnimationFrame(()=>this.world.processMeshQueue(this.quality.mobile?8:18));
    ...
    setTimeout(()=>{
      try{
        if(this.running && this.world.chunks.size>0 && this.world.meshes.size===0){
          ...
          this.world.processMeshQueue(this.quality.mobile?10:24);
 ...
 launchWorld(id){if(!id)return false;try{const data=SaveManager.readAll();const world=data.worlds?.[id];if(!world?.save)return false;SaveManager.setActive(id);for(const k of ["vs_launch_world_v73","vs_launch_world_v71","vs_launch_world_v70","vs_launch_world_v69","vs_launch_world_v68","vs_launch_world_v67"])try{sessionStorage.removeItem(k)}catch{}try{sessionStorage.setItem("vs_launch_world_v73",id)}catch{}location.reload();return true}catch(e){console.error("WORLD LAUNCH FAILED",e);return false}}
 newWorld(name="Новый мир",seed=""){this.skipUnloadSave=true;this.running=false;this.network.disconnect();const id=SaveManager.createWorld(name,seed);try{sessionStorage.setItem("vs_launch_world_v73",id)}catch(e){}location.reload()}
 ...
 setupTouchLayout(){
  this.touchLayoutKey="vs_touch_layout_v73";
  this.touchDefaults={
