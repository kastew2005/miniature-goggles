...
class Game{
 ...
 toggleInventory(){if(!this.running&&!this.inventoryOpen)return;if(this.inventoryUI){if(this.inventoryOpen)this.inventoryUI.close();else this.inventoryUI.open();return}this.inventoryOpen=!this.inventoryOpen}
 setupInventory(){this.craftGrid2=Array.from({length:4},()=>({id:0,count:0}));this.craftGrid3=Array.from({length:9},()=>({id:0,count:0}));this.craftMode="player";try{this.inventoryUI=new InventoryUI(this);this.inventoryUI.setupModeButtons();this.renderInventory()}catch(e){console.error("Inventory UI init failed; gameplay will continue",e);this.inventoryUI=null}}
 openCraftingTable(){this.craftMode="table";this.inventoryUI?.open();this.inventoryOpen=true;this.inventoryUI?.render()}
 returnCraftGrid(grid){if(!grid)return true;for(const s of grid){if(s.id&&s.count){const r=this.inventory.addDetailed(s.id,s.count);if(r.remaining>0)this.mining?.spawnDrop?.(s.id,r.remaining,{x:Math.floor(this.player.pos.x),y:Math.floor(this.player.pos.y),z:Math.floor(this.player.pos.z)});s.id=0;s.count=0}}return true}
 closeCraftingUI(){this.returnCraftGrid(this.craftMode==="table"?this.craftGrid3:this.craftGrid2);this.craftMode="player";this.inventoryOpen=false;document.getElementById("inventoryPanel")?.classList.add("hidden")}
 renderInventory(){this.inventoryUI?.render()}
 setupCraftingInventoryInteractions(){this.inventoryUI?.render()}
 ...
 useSelected(){const s=this.inventory.selectedItem(),info=s?.id?INFO[s.id]:null;if(info?.food){if(this.player.eat())this.audio.block();return}if(s?.id===ITEM.HOE){const hit=this.player.raycast();if(hit&&[BLOCK.DIRT,BLOCK.GRASS].includes(hit.id)&&this.world.getBlock(hit.block.x,hit.block.y+1,hit.block.z)===BLOCK.AIR){this.world.setBlock(hit.block.x,hit.block.y,hit.block.z,BLOCK.FARMLAND);this.audio.block();this.particles.dust(new THREE.Vector3(hit.block.x+.5,hit.block.y+.5,hit.block.z+.5),0x7a4c2b,8);this.systems.award(2,"Первая грядка")}return}if(info?.place||info?.solid)this.placeBlock()}
  attackOrBreak(){const target=this.player.raycast();if(this.systems.interact(target))return;if(this.mobs.shear(this.player)){this.audio.pickup?.();this.systems.award(5,"Стрижка овцы");return;}const result=this.mobs.attack(this.player);if(result.hit){this.audio.hit();this.effects.hit();this.stopMining();if(result.dead){if(result.drops){for(const [id,count] of result.drops)this.inventory.add(id,count)}else{this.inventory.add(ITEM.RAW_MEAT,1);if(Math.random()<.25)this.inventory.add(ITEM.APPLE,1)}this.systems.award(20,result.mob?.type?"Охотник на животных":"Охотник на нежить");this.particles.burst(this.player.pos.clone().add(new THREE.Vector3(0,1,0)),0xb84b4b,20);}return}this.beginMining(target)}
